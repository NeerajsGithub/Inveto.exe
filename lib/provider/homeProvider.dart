...
Future<void> downloadAndOpenExcelFile(BuildContext context, String userEmail, String storeName) async {
  final url = Uri.parse('https://clever-shape-81254.pktriot.net/stores/get-file');
  ...
  try {
    ...
    // Request permission (only for Android)
    ...
    // Pick save location (Windows/macOS/Linux)
    String? filePath;
    if (Platform.isWindows || Platform.isMacOS || Platform.isLinux) {
      filePath = await FilePicker.platform.saveFile(
        dialogTitle: 'Select Save Location',
        fileName: '$storeName.xlsx', // ✅ Correct usage
      );
      if (filePath == null) {
        Navigator.pop(context);
      ...
      // Use application storage for Android/iOS
      final directory = await getApplicationDocumentsDirectory();
      filePath = '${directory.path}/$storeName.xlsx';
    }
    ...
    if (response.statusCode == 200) {
      ...
      // Close the dialog before opening the file
      Navigator.pop(context);
      ...
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Download failed: ${response.reasonPhrase}')));
    }
  } catch (e) {
    Navigator.pop(context);
    print("Error: $e");
...
// Separate function to open the file (prevents UI blocking)
